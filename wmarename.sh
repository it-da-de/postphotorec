...
for i in $(find $Ordner -iname '*.wma' -size +100k ) ; do
	TRACK=""
	ALBUM=""
	TITEL=""
	ARTIST=""
	EXIF=$(exiftool -Track -Albumtitle -Title -AlbumArtist $i)
	RC=$?
	TRACK=$(printf "%02d" "$(echo "$EXIF" | grep '^Track' | cut -d\: -f2- | sed -e's/^\s*[0]*//g' -e's/\/.*//g')")
	ALBUM=$(echo "$EXIF" | grep '^Album Title' | cut -d\: -f2- | tr -d [:punct:] | sed -e's/^\s*//' | tr -s ' ' '_' )
	ARTIST=$(echo "$EXIF" | grep '^Album Artist' | cut -d\: -f2- | tr -d [:punct:] | sed -e's/^\s*//' | tr -s ' ' '_' )
	TITEL=$(echo "$EXIF" | grep '^Title' | cut -d\: -f2- | tr -d [:punct:] | sed -e's/^\s*//' | tr -s ' ' '_' )
	if [ "$ALBUM" = "" ] ; then ALBUM="$ARTIST" ; fi
	echo "$TRACK $ALBUM $TITEL $ARTIST"
	if [ $RC -eq 0 ] && [ "$ALBUM" != "" ] ; then
		if ! [ -d $Ordner/$ALBUM.$ARTIST ] ; then  mkdir $Ordner/$ALBUM.$ARTIST ; fi
		mv $i $Ordner/$ALBUM.$ARTIST/${TRACK}-$TITEL.$(basename $i)
	fi
done
exit
cat << EOF >/dev/null
